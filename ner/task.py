...
class NERTask(object):
    ...
    def attack(self, config: ProgramArgs):
        with torch.no_grad():
            ...
            print("adv metric: {}".format(adv_metrics))
                # print("clean metric (Real): {}".format(clean_metrics_remove_unsatisfactory_f1score))
                # print("adv metric (Real): {}".format(adv_metrice_remove_unsatisfactory_f1score))
                    
                # whether current instance is unsatisfied to be attacked
                # unsatisfactory condition: sentence is shorter than config.min_sentence_length 
                # or there is no place to be attacked (all word in sentence are named entities)
            #     gold_tags = instance.fields['tags'].labels
            #     clean_result = utils.to_tensor(predictor.predict(instance))
            #     
            #     adv_result_final = clean_result
            #     
            #     if len(instance.fields['tokens']) > config.min_sentence_length:
            #         adv_instances = attacker.attack(instance)
            #         if adv_instances is not None:
            #             statisfied_flag = True            
            #             clean_metric_accuracy = utils.get_metric(self.vocab,
            #                                                      clean_result['logits'], 
            #                                                      clean_result['tags'], 
            #                                                      gold_tags, 
            #                                                      clean_result['mask'],
            ...
            #                                                      label_encoding=config.label_encoding)
            # 
            #             adv_results = predictor.predict(adv_instances)
            #             adv_metric_final = None
            #             for index, adv_result in enumerate(adv_results):
            #                 adv_result = utils.to_tensor(adv_result)
            #                 adv_metric = utils.get_metric(self.vocab,
            #                                               adv_result['logits'], 
            #                                               adv_result['tags'], 
            #                                               gold_tags, 
            #                                               adv_result['mask'],
            ...
            #                                               label_encoding=config.label_encoding)
            # 
            #                 if clean_metric_accuracy > adv_metric and (adv_metric_final == None or adv_metric < adv_metric_final):
            #                     succeed_flag = True
            #                     adv_metric_final = adv_metric
            #                     adv_result_final = adv_result
            #                     if adv_metric_final.accuracy == 0:
            #                         break
            #     
            #     clean_metric_f1 = utils.get_metric(self.vocab,
            #                                        clean_result['logits'], 
            #                                        clean_result['tags'], 
            #                                        gold_tags, 
            #                                        clean_result['mask'],
            #                                        metric_type='f1',
            #                                        label_encoding=config.label_encoding)
            #     adv_metric_f1 = utils.get_metric(self.vocab,
            #                                      adv_result_final['logits'], 
            #                                      adv_result_final['tags'], 
            #                                      gold_tags, 
            #                                      adv_result_final['mask'],
            #                                      metric_type='f1',
            #                                      label_encoding=config.label_encoding)
            #     
            #     clean_metrics_f1score += clean_metric_f1
            #     adv_metrics_f1score += adv_metric_f1
            #     if statisfied_flag:
            #         all_remove_unsatisfactory += 1
            #         clean_metrics_remove_unsatisfactory_f1score += clean_metric_f1
            #         adv_metrice_remove_unsatisfactory_f1score += adv_metric_f1
            #         if succeed_flag:
            #             succeed_number += 1
            #             print("\n(clean) tokens: {} tags: {}".format(' '.join(clean_result['words']),
            #                                                          ' '.join(clean_result['tags'])))
            #             print("(adv)   tokens: {} tags: {}".format(' '.join(adv_result_final['words']),
            #                                                        ' '.join(adv_result_final['tags'])))
            #             tqdm_bar.set_description("Succeed! clean:{:.4f} adv:{:.4f}".format(clean_metric_accuracy.accuracy, adv_metric_final.accuracy))
            #         else:
            #             print("\n(clean) tokens: {} tags: {}".format(' '.join(clean_result['words']),
            #                                                          ' '.join(clean_result['tags'])))
            #             tqdm_bar.set_description("Fail! clean:{:.4f}".format(clean_metric_accuracy.accuracy))
            #     else:
            #         print("\n(clean) tokens: {} tags: {}".format(' '.join(clean_result['words']),
            #                                                      ' '.join(clean_result['tags'])))
            #         tqdm_bar.set_description("Not Attack! clean:{:.4f}".format(clean_metric_accuracy.accuracy))
            # 
            # print("sentence numbers: {}, among them, {} are Unsatisfactory".format(len(self.test_data), len(self.test_data) - all_remove_unsatisfactory))
            # print("Success Rate: {:.2f}%".format(succeed_number / len(self.test_data) * 100))
            # print("Success Rate (Real): {:.2f}%".format(succeed_number / (all_remove_unsatisfactory) * 100))
            # print("clean metric: {}".format(clean_metrics_f1score))
            # print("adv metric: {}".format(adv_metrics_f1score))
            # print("clean metric (Real): {}".format(clean_metrics_remove_unsatisfactory_f1score))
            # print("adv metric (Real): {}".format(adv_metrice_remove_unsatisfactory_f1score))
    #             
    #             
    #             if adv_instances is None:
    #                 print("\n(clean) tokens: {} tags: {}".format(' '.join(clean_result['words']),' '.join(clean_result['tags'])))
    #                 tqdm_bar.set_description("Fail! clean:{:.4f} adv:{:.4f}".format(clean_metric.f1_score, clean_metric.f1_score))
    #                 clean_metrics_f1score += clean_metric_f1score
    #                 adv_metrics += clean_metric
    #                 continue
    #                 
    #             all_remove_unsuitable += 1
    #             adv_results = predictor.predict(adv_instances)
    #             
    #             succeed_flag = False
    #             adv_result_min = None
    ...
    #             
    #             for index, adv_result in enumerate(adv_results):
    #                 adv_result = to_tensor(adv_result)
    #                 adv_metric = self.get_metric(config, adv_result['logits'], adv_result['tags'], gold_tags, adv_result['mask'])
    #                 
    #                 if clean_metric > adv_metric and (adv_metric_min == None or adv_metric < adv_metric_min):
    #                     succeed_flag = True
    #                     adv_metric_min = adv_metric
    #                     adv_result_min = adv_result
    #                     if adv_metric_min.f1_score == 0:
    #                         break
    #                         
    #             if succeed_flag: 
    #                 succeed_number += 1
    #                 print("\n(clean) tokens: {} tags: {}".format(' '.join(clean_result['words']),' '.join(clean_result['tags'])))
    #                 print("(adv)   tokens: {} tags: {}".format(' '.join(adv_result_min['words']),' '.join(adv_result_min['tags'])))
    #                 tqdm_bar.set_description("Succeed! clean:{:.4f} adv:{:.4f}".format(clean_metric.f1_score, adv_metric_min.f1_score))
    #             
    #                 clean_metrics += clean_metric
    #                 adv_metrics += adv_metric_min
    #                 
    #                 clean_metrics_remove_unsuitable += clean_metric
    #                 adv_metrice_remove_unsuitable += adv_metric_min
    #             else:
    #                 print("\n(clean) tokens: {} tags: {}".format(' '.join(clean_result['words']),' '.join(clean_result['tags'])))
    #                 clean_metrics += clean_metric
    #                 adv_metrics += clean_metric
    # 
    #                 clean_metrics_remove_unsuitable += clean_metric
    #                 adv_metrice_remove_unsuitable += clean_metric
    #                 tqdm_bar.set_description("Fail! clean:{:.4f} adv:{:.4f}".format(clean_metric.f1_score, clean_metric.f1_score))
