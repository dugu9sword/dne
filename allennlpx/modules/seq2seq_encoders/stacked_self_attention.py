...
class StackedSelfAttentionEncoder(Seq2SeqEncoder):
    ...
    def __init__(
        ...
        self._input_layernorm = LayerNorm(input_dim)
    ...
    def forward(self, inputs: torch.Tensor, mask: torch.Tensor):
        if self._use_positional_encoding:
            ...
            output = inputs
        output = self._input_layernorm(output)
        for i in range(len(self._attention_layers)):
            ...
            output = output + self.dropout(attention(layer_norm(output), mask))
            output = output + self.dropout(feedforward(feedforward_layer_norm(output)))
            ...
            # output = output + feedforward_layer_norm(output)
            # feedforward_output = feedforward_layer_norm(output)
            # feedforward_output = feedforward(feedforward_output)
            ...
            # feedforward_output = feedforward_output + cached_input
            ...
            # output = self.dropout(attention_output) + attention_output
        return output
