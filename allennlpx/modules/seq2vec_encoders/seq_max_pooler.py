...
class PytorchSeqMaxPooler(Seq2VecEncoder):
    """
    ...
    def forward(
        ...
        outputs, state, restoration_indices, = self.sort_and_run_forward(
            self._module, inputs, mask, hidden_state
        )
        outputs, _ = pad_packed_sequence(outputs, batch_first=True, padding_value=-1000.)
...
#         return last_layer_state.contiguous().view([-1, self.get_output_dim()])
        return unsorted_outputs.max(1)[0]